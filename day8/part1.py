...
def main(inp: List[Node]):
    ...
    for u in range(len(inp)):
        v = unionFind.find(u)
        if v not in componentSize:
            componentSize[v] = 1
        if u != v:
            componentSize[v] += 1

    largest_components = [1, 1, 1]
    for v in componentSize.values():
        idx = bisect.bisect_right(largest_components, v)
        largest_components.insert(idx, v)
        largest_components.pop(0)
